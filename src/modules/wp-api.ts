import type { HomeData, SideBarDto } from './wp-api-types';
...
export async function getWpSideBar(): Promise<SideBarDto> {
  const res = await fetch(`https://telegraf.news/api/v1/sidebar.php`);
  if (!res.ok) throw new Error('Home not found');
  const raw = await res.json(); // вместо res.text()
  const data = raw as SideBarDto;
